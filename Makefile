...
#
# Sources/MetalTileSwift/Generated/ is auto-regenerated by
# `tile build --emit` in the sibling metaltile repo; any formatting
# fixes we'd apply there would be blown away on the next regen. The
# upstream metaltile codegen owns those warnings. Xcode-bundled
# swift-format 6.3 doesn't honor `.swift-format-ignore`, so the
# excludes go in the file-enumeration here.
SWIFT_FORMAT_FILES := $(shell find Sources Tests -name '*.swift' \
    -not -path 'Sources/MetalTileSwift/Generated/*')

.PHONY: format
format: ## run swift-format on all .swift files (skips auto-generated)
	xcrun swift-format format --in-place --configuration .swift-format $(SWIFT_FORMAT_FILES)

.PHONY: format-check
format-check: ## swift-format lint (no writes; skips auto-generated)
	xcrun swift-format lint --configuration .swift-format $(SWIFT_FORMAT_FILES) && echo "format OK"
