...
//             * Re-run `forward(lastAccepted at pos)` to advance caches
//               by 1 token (the previously-correct last accepted).
//             * Then `forward(c_0 at pos+1)` ... `forward(c_{k-1} at
//               pos+k)` to advance through the accepted prefix.
//             * Commit accepted prefix [c_0..c_{k-1}] + the model's
//               actual token at position pos+1+k = argmax(logits[k]).
//             * tokens committed this iter: k + 1.
//             * pos advances by k + 1.
//           If no mismatch (all γ' accepted):
//             * Commit [c_0..c_{γ'-1}] + the bonus token from the trailing
//               logits[γ'] (the model's prediction for what comes AFTER
//               the last candidate).
...
//
// Expected wins on Qwen3.6-A3B M5 Max (60 ms decode baseline):
//   acceptance α = 0.5 → ~16.7 tps (break-even)
//   acceptance α = 0.7 → ~21.8 tps (~1.3×)
//   acceptance α = 1.0 → ~33.3 tps (2×)
// γ > 1 versions push the ceiling higher when the drafter is accurate.
...
public enum SpecDecode {
    ...
    public static func generateGreedy(
        ...
        precondition(maxNewTokens >= 0, "SpecDecode.generateGreedy: maxNewTokens must be ≥ 0")
        ...
        loop: while tokensGenerated < maxNewTokens {
            ...
            if candidates.isEmpty {
                ...
                let logits = model.forward(tokenId: prev, position: pos,
            ...
            precondition(flat.count == (gp + 1) * vocab,
            ...
            if firstMismatchTok != nil {
                // ── Partial accept (could be 0): restore + replay accepted prefix ─
                // We DO NOT trust the batched verify's logits for sampling
                // the next-prev — forwardManyAllLogits can drift slightly
                // (different kernel paths, accumulation order) vs single
                // forward(). After replay, take the next-prev from the
                // LAST single-step forward — that is bit-identical to
                // what the baseline greedy loop produces.
                ...
                for (i, tok) in toReplay.enumerated() {
                    ...
                    lastLogits = model.forward(tokenId: tok, position: pos + i,
                ...
                // ── Full accept: commit all γ' candidates + bonus token ─
                // Bonus token = argmax(logits[gp]) — model's prediction at position pos + gp + 1.
                let bonusStart = gp * vocab
                let bonusTok = argmax(flat, offset: bonusStart, count: vocab)
                history.append(prev)
                for c in candidates { history.append(c) }
                let totalCommitted = 1 + gp  // prev + all candidates (bonus becomes next prev)
                pos += totalCommitted
                prev = bonusTok
                tokensGenerated += totalCommitted
