...
func lfm2vlLoadVisionEncoder(
    ...
    let patchW = reshapeLinearPatchEmbed(
        patchWRaw,
        hidden: hidden,
        channels: inChannels,
        patchSize: patchSize)
...
private func reshapeLinearPatchEmbed(
    ...
    precondition(
        src.shape == [hidden, channels * patchSize * patchSize],
    ...
    memcpy(
        dst.buffer.contents().advanced(by: dst.offset),
...
public final class LFM2VLProjector: @unchecked Sendable {
    ...
    init(
        layerNorm: LayerNorm, linear1: AnyLinear, linear2: AnyLinear,
    ...
    static func load(
        ...
        let b1: Tensor? =
            weights.has("linear_1.bias")
            ? try weights.tensor(named: "linear_1.bias")
            : nil
        let b2: Tensor? =
            weights.has("linear_2.bias")
...
final class LFM2VLComposedTower {
    ...
    init(
        encoder: VisionEncoder, projector: LFM2VLProjector,
...
func lfm2LoadModelQuantized(
    ...
    for (i, kind) in kinds.enumerated() {
        ...
        let ffn = LFM2FFN.dense(
            LFM2MLP(
                w1: try loadLinear(
                    base: "\(p).feed_forward.w1", in: weights, quantization: quantization),
                w3: try loadLinear(
                    base: "\(p).feed_forward.w3", in: weights, quantization: quantization),
                w2: try loadLinear(
                    base: "\(p).feed_forward.w2", in: weights, quantization: quantization)))
...
private func lfm2vlWriteFloats(
