...
public struct FireRedASR2EncoderConfig: Sendable {
    ...
    public init(nLayers: Int = 16, nHead: Int = 20, dModel: Int = 1280,
...
public struct FireRedASR2Config: Sendable {
    ...
    public init(modelType: String = "fireredasr2",
    ...
    public static func from(_ config: ModelConfig) -> FireRedASR2Config? {
        ...
        guard FireRedASR2Model.modelTypes.contains(mt)
...
extension FireRedASR2Model {
    ...
    static func kaldiFbank(waveform: [Float], idim: Int) -> [Float] {
        ...
        let melBank = AudioPreprocessing.melFilterbank(
            AudioFrontEndConfig(sampleRate: fbankSampleRate,
                                nFFT: fftLength,
                                hopLength: hopLength,
                                nMels: nMels,
                                fMin: 20.0))
    ...
    private func applyCMVN(_ feats: inout [Float], dim: Int) {
        guard let means = cmvnMeans, let istd = cmvnIstd,
              means.count == dim, istd.count == dim else { return }
    ...
    public func encodeAudio(waveform: [Float],
        ...
        x = Ops.conv2d(input: x, weight: conv2Weight,
        ...
        let xFlat = Tensor.empty(shape: [hOut, flatDim], dtype: dtype,
                                 device: device)
        ...
        let projected = Ops.gemm(weight: convOutWeight, input: xFlat,
                                 nRows: hOut, on: cmd3)
        ...
        for block in encoderBlocks {
            seqVals = runConformerBlock(block, seq: seqVals,
                                        posEmb: posEmb,
                                        seqLen: T, dModel: dModel)
        }

        let result = Tensor.empty(shape: [T, dModel], dtype: dtype,
                                  device: device)
    ...
    private func runConformerBlock(_ block: FireRedASR2ConformerBlock,
                                   seq seqIn: [Float],
                                   posEmb: [Float],
                                   seqLen T: Int,
                                   dModel: Int) -> [Float] {
        ...
        h = runRelPosAttn(block.mhsa, seq: h,
    ...
    private func runFFN(_ ffn: FireRedASR2FFN, seq seqIn: [Float],
                        seqLen T: Int, dModel: Int) -> [Float] {
        // norm0
    ...
    private func runRelPosAttn(_ attn: FireRedASR2RelPosAttn,
                                seq seqIn: [Float],
                                posEmb: [Float],
                                seqLen T: Int,
                                dModel: Int) -> [Float] {
        ...
        for head in 0..<nHead {
            ...
            for i in 0..<T {
                for j in 0..<T {
                    ...
                    for d in 0..<dK {
                        dot += (qProj[qBase + d] + attn.posBiasU[buOff + d])
                             * kProj[kBase + d]
                    }
            ...
            for i in 0..<T {
                for p in 0..<peLen {
                    ...
                    for d in 0..<dK {
                        dot += (qProj[qBase + d] + attn.posBiasV[bvOff + d])
                             * pProj[pBase + d]
                    }
    ...
    private func runConformerConv(_ cb: FireRedASR2ConvBlock,
                                   seq seqIn: [Float],
                                   seqLen T: Int, dModel: Int) -> [Float] {
        // pre_layer_norm
    ...
    private func linearRows(_ linear: Linear, rows: [Float],
    ...
    private func layerNormRows(_ ln: LayerNorm, rows: [Float],
                                seqLen nRows: Int, dim: Int) -> [Float] {
...
extension FireRedASR2Model {
    ...
    private func beamSearch(
        ...
        var expandedEnc = [Float](repeating: 0,
        ...
        var beamScores = [Float](repeating: -Float.greatestFiniteMagnitude,
                                 count: beamCount)
        beamScores[0] = 0
        ...
        for _ in 0..<maxLen {
            ...
            for b in 0..<beamCount {
                if beamFinished[b] {
                    ...
                    candidates.append(Candidate(beamIdx: b, token: eosID,
                                                score: 0,
                ...
                for layer in decoderLayers {
                    embSeq = runDecoderLayer(layer, seq: embSeq,
                                             encOut: encSlice,
                                             seqLen: seqLen,
                ...
                let normedSeq = layerNormRows(decoderNormOut, rows: embSeq,
                ...
                let logProbs = logSoftmax(logits, temperature: softmaxSmoothing,
    ...
    private func embedTokens(_ tokenIds: [Int], scale: Float,
                              dModel: Int) -> [Float] {
        ...
        for (t, id) in tokenIds.enumerated() {
            ...
            for d in 0..<dModel {
                out[dstBase + d] = embVals[srcBase + d] * scale
    ...
    private func projectToVocab(_ hidden: [Float],
                                 dModel: Int, odim: Int) -> [Float] {
    ...
    private func logSoftmax(_ logits: [Float], temperature: Float,
                             eosPenalty: Float, eosID: Int) -> [Float] {
        let n = logits.count
    ...
    private func runDecoderLayer(_ layer: FireRedASR2DecoderLayer,
                                  seq seqIn: [Float],
                                  encOut: [Float],
                                  seqLen: Int,
                                  encLen: Int,
        ...
        let normSelf = layerNormRows(layer.selfAttnNorm, rows: seqIn,
                                     seqLen: seqLen, dim: dModel)
        let selfAttnOut = causalSelfAttn(layer.selfAttn,
        ...
        let normCross = layerNormRows(layer.crossAttnNorm, rows: h,
                                      seqLen: seqLen, dim: dModel)
        let crossAttnOut = crossAttn(layer.crossAttn,
        ...
        // 3. FFN sub-block.
        let normMLP = layerNormRows(layer.mlpNorm, rows: h2,
                                    seqLen: seqLen, dim: dModel)
        let w1Out = linearRows(layer.w1, rows: normMLP, inDim: dModel,
                               outDim: dModel * 4)
    ...
    private func causalSelfAttn(_ attn: FireRedASR2CrossAttn,
                                 seq: [Float], seqLen: Int,
                                 stride: Int, nHead: Int, dK: Int) -> [Float] {
        ...
        return linearRows(attn.fc, rows: out, inDim: stride,
                          outDim: config.decoder.dModel)
    }

    /// Cross-attention: queries from decoder, keys/values from encoder.
    private func crossAttn(_ attn: FireRedASR2CrossAttn,
                            q: [Float], qLen: Int,
                            kv: [Float], kvLen: Int,
                            stride: Int, nHead: Int, dK: Int) -> [Float] {
...
extension FireRedASR2Model {
    ...
    public static func load(directory: URL,
                            device: Device = .shared) throws -> FireRedASR2Model {
        ...
        return try build(config: fc, bundle: bundle, directory: directory,
                         device: device)
    }
    ...
    public static func build(
        ...
        if let dir = directory {
            ...
            if FileManager.default.fileExists(atPath: cmvnURL.path),
               ...
               let decoded = try? JSONDecoder().decode(FireRedASR2CMVN.self,
                                                        from: data) {
        ...
        let tgtWordPrj = bundle.has("decoder.tgt_word_prj.weight")
        ...
        for i in 0..<fc.decoder.nLayers {
            ...
            let crossAttn = FireRedASR2CrossAttn(
                ...
                nHead: decNHead, dK: decDK)
            decoderLayers.append(FireRedASR2DecoderLayer(
