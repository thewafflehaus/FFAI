...
public struct SenseVoiceFrontEndConfig: Sendable {
    ...
    public init(sampleRate: Int = 16_000, nMels: Int = 80,
...
public struct SenseVoiceConfig: Sendable {
    ...
    public init(vocab: Int, inputSize: Int, hidden: Int, heads: Int,
...
public final class SenseVoiceEncoderLayer: Module {
    ...
    init(norm1: LayerNorm, norm2: LayerNorm, qkvProj: Linear,
        ...
        self.qkvProj = qkvProj; self.outProj = outProj
        self.fsmnWeight = fsmnWeight
        self.w1 = w1; self.w2 = w2
        self.inSize = inSize; self.hidden = hidden
        self.heads = heads; self.headDim = hidden / heads
        self.fsmnKernel = fsmnKernel
...
public final class SenseVoiceModel: @unchecked Sendable {
    ...
    public init(config: SenseVoiceConfig, queryEmbed: Tensor,
    ...
    public func ctcLogProbs(waveform: [Float], language: Int = 0,
        ...
        var rows = [Float]()
        rows.reserveCapacity((prefixIds.count + nFeatFrames)
                             * config.inputSize)
        ...
        let logits = projectRows(ctcProj, h, nRows: nRows,
                                 outDim: config.vocab, on: cmd)
        cmd.commit()
        cmd.waitUntilCompleted()
        return logSoftmaxRows(logits, nRows: nRows, rowSize: config.vocab,
                              device: device)
    }
    ...
    public func transcribeTokens(waveform: [Float], language: Int = 0,
                                 device: Device = .shared) -> [Int] {
        let logProbs = ctcLogProbs(waveform: waveform, language: language,
                                   device: device)
    ...
    private func encode(rows: [Float], nRows: Int,
                        device: Device) -> Tensor {
        let inputT = Tensor.empty(shape: [nRows, config.inputSize],
        ...
        h = addSinusoidalPositions(h, nRows: nRows,
    ...
    private func forwardLayer(_ layer: SenseVoiceEncoderLayer, h hIn: Tensor,
                              nRows: Int, device: Device) -> Tensor {
        let H = config.hidden

        // ── Self-attention sub-block ──
        let cmd1 = device.makeCommandBuffer()
        let normed = Ops.layerNorm(hIn, weight: layer.norm1.weight,
                                   bias: layer.norm1.bias,
                                   eps: layer.norm1.eps,
                                   nRows: nRows, rowSize: layer.inSize,
                                   on: cmd1)
        // Fused Q/K/V projection → [nRows, 3*hidden].
        let qkv = projectRows(layer.qkvProj, normed, nRows: nRows,
                              outDim: 3 * H, on: cmd1)
        cmd1.commit()
        ...
        let ctx = cpuAttention(qa: qa, ka: ka, va: va, nRows: nRows,
                               heads: layer.heads, headDim: layer.headDim,
                               scale: layer.scale, device: device)

        let cmd2 = device.makeCommandBuffer()
        var attnOut = projectRows(layer.outProj, ctx, nRows: nRows,
                                  outDim: H, on: cmd2)
        // attnOut += FSMN memory.
        let memT = Tensor.empty(shape: [nRows, H], dtype: dtype,
                                device: device)
        ...
        var h = layer.inSize == H
            ? Ops.add(hIn, attnOut, on: cmd2)
            : attnOut

        // ── ReLU MLP sub-block ──
        let mlpNormed = Ops.layerNorm(h, weight: layer.norm2.weight,
                                      bias: layer.norm2.bias,
                                      eps: layer.norm2.eps,
                                      nRows: nRows, rowSize: H, on: cmd2)
        let ff1 = projectRows(layer.w1, mlpNormed, nRows: nRows,
                              outDim: layer.w1.weight.shape[0], on: cmd2)
        let act = Ops.relu(ff1, on: cmd2)
        let ff2 = projectRows(layer.w2, act, nRows: nRows,
                              outDim: H, on: cmd2)
    ...
    private func fsmn(_ layer: SenseVoiceEncoderLayer, v: [Float],
                      nRows: Int) -> [Float] {
        let H = config.hidden
    ...
    private func applyCMVN(_ feats: inout [Float]) {
        guard let mean = cmvnMean, let invStd = cmvnInvStd,
              ...
              invStd.count == config.inputSize else { return }
    ...
    private func projectRows(_ linear: Linear, _ x: Tensor, nRows: Int,
        ...
        let y = Ops.gemm(weight: linear.weight, input: x, nRows: nRows,
                         on: cmd)
    ...
    private func normRows(_ ln: LayerNorm, _ x: Tensor, nRows: Int,
                          device: Device) -> Tensor {
        let cmd = device.makeCommandBuffer()
        let y = Ops.layerNorm(x, weight: ln.weight, bias: ln.bias,
    ...
    private func scaleRows(_ x: Tensor, by factor: Float,
                           on cmd: MTLCommandBuffer) -> Tensor {
    ...
    private func addSinusoidalPositions(_ x: Tensor, nRows: Int, dim: Int,
                                        device: Device) -> Tensor {
        ...
        let posT = Tensor.empty(shape: [nRows, dim], dtype: dtype,
                                device: device)
    ...
    private func logSoftmaxRows(_ logits: Tensor, nRows: Int, rowSize: Int,
                                device: Device) -> Tensor {
...
public enum SenseVoiceFrontEnd {
    ...
    public static func featureFrames(waveform: [Float],
                                     cfg: SenseVoiceFrontEndConfig)
        -> [Float] {
        let fbank = kaldiFbank(waveform: waveform, cfg: cfg)
        return lowFrameRate(fbank, nMels: cfg.nMels,
    ...
    static func kaldiFbank(waveform: [Float],
                           cfg: SenseVoiceFrontEndConfig) -> [Float] {
        ...
        for f in 0..<nFrames {
            ...
            let power = realFFTPower(frame, fftLength: fftLength,
                                     nFreq: nFreq)
    ...
    static func lowFrameRate(_ fbank: [Float], nMels: Int,
                             lfrM: Int, lfrN: Int) -> [Float] {
        ...
        for r in 0..<lfrFrames {
            ...
            for m in 0..<lfrM {
                ...
                for v in slice { out[offset] = v; offset += 1 }
            }
        }
        return out
    }
    ...
    /// encoder.
    static func realFFTPower(_ frame: [Float], fftLength: Int,
                             nFreq: Int) -> [Float] {
        var re = frame
    ...
    private static func fftInPlace(_ re: inout [Float],
                                   _ im: inout [Float]) {
        let n = re.count
...
extension SenseVoiceModel {
    ...
    public static let modelTypes: Set<String> = ["sensevoice",
                                                  "sense_voice"]
    ...
    public static func handles(_ config: ModelConfig) -> Bool {
        if let mt = config.modelType,
           modelTypes.contains(mt.lowercased()) { return true }
        if let arch = config.architecture,
           architectures.contains(arch) { return true }
    ...
    public static func load(directory: URL, device: Device = .shared)
        ...
        return try build(config: sc, bundle: bundle,
    ...
    private static func parseAMMVN(_ text: String)
        ...
        guard let mean = bracketed(after: "<AddShift>"),
              ...
              !mean.isEmpty, mean.count == invStd.count else { return nil }
        return (mean, invStd)
    }
}
