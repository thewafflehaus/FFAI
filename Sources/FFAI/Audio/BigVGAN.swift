...
public struct BigVGANConfig: Codable, Sendable {
    ...
    public init(from decoder: Swift.Decoder) throws {
        ...
        upsampleRates = try c.decodeIfPresent([Int].self, forKey: .upsampleRates)
            ?? [4, 4, 2, 2, 2, 2]
        upsampleKernelSizes = try c.decodeIfPresent([Int].self, forKey: .upsampleKernelSizes)
            ?? [8, 8, 4, 4, 4, 4]
        upsampleInitialChannel = try c.decodeIfPresent(Int.self, forKey: .upsampleInitialChannel)
            ?? 1536
        resblock = try c.decodeIfPresent(BigVGANResBlockType.self, forKey: .resblock) ?? .one
        resblockKernelSizes = try c.decodeIfPresent([Int].self, forKey: .resblockKernelSizes)
            ?? [3, 7, 11]
        resblockDilationSizes = try c.decodeIfPresent([[Int]].self, forKey: .resblockDilationSizes)
            ?? [[1, 3, 5], [1, 3, 5], [1, 3, 5]]
        activation = try c.decodeIfPresent(BigVGANActivationType.self, forKey: .activation)
            ?? .snakebeta
...
struct BigVGANWeights {
    ...
    func wnConvTranspose1d(prefix: String, stride: Int, padding: Int,
                           groups: Int) throws -> SNACWNConvTranspose1d {
        let gKey = "\(prefix).weight_g"
        let vKey = "\(prefix).weight_v"
        guard has(gKey), has(vKey) else {
            throw BigVGANError.missingWeights(prefix)
...
public final class BigVGAN: @unchecked Sendable {
    ...
    public static func fromPretrained(directory: URL) throws -> BigVGAN {
        ...
        let config = try JSONDecoder().decode(BigVGANConfig.self,
    ...
    init(config: BigVGANConfig, bundle: SafeTensorsBundle) throws {
        ...
        self.convPre = try w.wnConv1d(prefix: "conv_pre", stride: 1,
                                      padding: 3, dilation: 1, groups: 1)

        // Upsample stages — WNConvTranspose1d, halving channels each step.
        var ups: [SNACWNConvTranspose1d] = []
        for (i, pair) in zip(config.upsampleRates,
                             config.upsampleKernelSizes).enumerated() {
            let (stride, ksize) = pair
            ups.append(try w.wnConvTranspose1d(
        ...
        for s in 0..<config.numUpsamples {
            ...
            for k in 0..<config.numKernels {
                ...
                blocks.append(try BigVGANAMPBlock(
        ...
        let finalChannels = config.upsampleInitialChannel
        ...
        self.convPost = try w.wnConv1d(prefix: "conv_post", stride: 1,
                                       padding: 3, dilation: 1, groups: 1)
    }
    ...
    public func decode(mel: Tensor) throws -> Tensor {
        ...
        for step in 0..<config.numUpsamples {
            ...
            for k in 1..<config.numKernels {
                ...
                precondition(out.count == acc.count,
