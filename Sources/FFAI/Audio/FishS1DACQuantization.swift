...
enum FishS1RoPE {
    ...
    /// of headDim.
    static func apply(
        _ x: [Float], t: Int, nHead: Int, headDim: Int,
        base: Double
    ) -> [Float] {
        let half = headDim / 2
        var out = x
...
struct FishS1DACTransformerBlock {
    ...
    func callAsFunction(_ x: [Float], t: Int, windowSize: Int) -> [Float] {
        ...
        let proj = AudioMath.linear(
            attnOut, rows: t, inDim: qSize,
...
struct FishS1DACWindowLimitedTransformer {
    ...
    func forward(_ x: [Float], shape: [Int]) -> (data: [Float], shape: [Int]) {
        ...
        var hidden: [Float]
        if let w = inputProjW {
            hidden = AudioMath.linear(
                seq, rows: t, inDim: inputDim,
        ...
        if let w = outputProjW {
            outSeq = AudioMath.linear(
                hidden, rows: t, inDim: dim,
...
struct FishS1DACDownsampleStage {
    ...
    init(
        weights w: FishS1DACWeights, prefix: String,
        inputDim: Int, outputDim: Int, factor: Int
    ) throws {
        ...
        self.conv = SNACWNConv1d(
            weight: rawW, wShape: rawWShape, bias: bias,
    ...
    func callAsFunction(_ x: [Float], shape: [Int]) -> (data: [Float], shape: [Int]) {
        ...
        let (padded, paddedShape) = AudioMath.zeroPad1d(
            x, shape: shape,
...
struct FishS1DACUpsampleStage {
    ...
    init(
        ...
        self.conv = SNACWNConvTranspose1d(
            weight: rawW, wShape: rawWShape, bias: bias,
            stride: factor, padding: 0,
...
struct FishS1DACDownsampleRVQ {
    ...
    private static func loadTransformer(
        ...
        if inputDim != dim {
            inputProjW =
                w.has("\(prefix).input_proj.weight")
                ? try w.floats("\(prefix).input_proj.weight") : nil
            outputProjW =
                w.has("\(prefix).output_proj.weight")
