// Slow integration test for Qwen 2.5-VL's *video* path
// (`Qwen2_5_VLForConditionalGeneration` checkpoint, 7B-Instruct-4bit).
//
// Verifies the end-to-end multi-frame video-inference path on a real
// checkpoint: the Qwen 2.5-VL vision tower loads, runs its windowed-
// attention + M-RoPE forward over a stack of preprocessed frames with
// temporal-patch folding (each `temporal_patch_size` consecutive frames
// fold into one temporal patch), the cross-modal splice injects the
// merged video tokens at every `<|video_pad|>` placeholder, and the
// fused stream decodes coherent text through the Qwen 2.x backbone.
//
// The checkpoint download is ~10 GB; this test is gated by the standard
...
struct Qwen25VLVideoIntegrationTests {
    ...
    static let modelId = "mlx-community/Qwen2.5-VL-7B-Instruct-4bit"
