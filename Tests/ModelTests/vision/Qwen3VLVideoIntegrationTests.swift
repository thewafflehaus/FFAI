// Slow integration test for Qwen 3-VL's *video* path
// (`Qwen3VLForConditionalGeneration` checkpoint, 2B-Instruct-4bit).
//
// Verifies the end-to-end multi-frame video-inference path on a real
// checkpoint: the Qwen 3-VL vision tower loads, runs its full-attention
...
// decodes coherent text through the Qwen 3 backbone.
//
...
struct Qwen3VLVideoIntegrationTests {
    ...
    func videoTextGeneration() async throws {
        ...
        VLMTestSupport.expectMentionsCat(text, label: "Qwen 3-VL video")
    }
}
