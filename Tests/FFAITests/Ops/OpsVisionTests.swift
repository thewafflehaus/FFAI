...
struct OpsVisionTests {
        ...
        for token in 0 ..< nTokens {
            ...
            for head in 0 ..< nHeads {
                ...
                for j in 0 ..< quarterDim {
                    ...
                    let cosR = cos(row * invFreq)
                    let sinR = sin(row * invFreq)
                    let cosC = cos(col * invFreq)
                    let sinC = sin(col * invFreq)
                    let xr1 = qk[base + j]
                    let xr2 = qk[base + j + quarterDim]
    ...
    func rope2DF32() {
        autoreleasepool {
            ...
            runAndWait { cb in
                out = Ops.rope2D(
                    qk, positions: pos, nTokens: nTokens,
    ...
    func layerNormF32() {
        autoreleasepool {
            ...
            runAndWait { cb in
                out = Ops.layerNorm(
                    x, weight: weight, bias: bias, eps: 1e-5,
