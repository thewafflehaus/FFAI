...
struct SSMStateCacheTests {
    ...
    func singleStepMatchesCPU() {
        autoreleasepool {
            let nHeads = 2, stateDim = 4, headDim = 8
            ...
            let (gpuH, gpuY) = gpuStep(x: x, a: a, b: b, c: c, dt: dt,
    ...
    func multiStepMatchesCPU() {
        autoreleasepool {
            let nHeads = 4, stateDim = 8, headDim = 16
    ...
    func bf16InputsMatchCPU() {
        autoreleasepool {
            ...
            aT.copyIn(from: a.map { f -> UInt16 in
                UInt16(truncatingIfNeeded: f.bitPattern >> 16)
            })
            ...
            let gpuH = cache.h.toArray(as: Float.self)
            for i in 0..<cpuH.count {
                #expect(abs(gpuH[i] - cpuH[i]) < stateTol,
                        "h[\(i)] gpu=\(gpuH[i]) cpu=\(cpuH[i])")
            }
            ...
            for i in 0..<(nHeads * headDim) {
                ...
                #expect(abs(gpuYf - expected) < outTol,
