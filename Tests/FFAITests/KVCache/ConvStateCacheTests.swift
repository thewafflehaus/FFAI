...
struct ConvStateCacheTests {
    ...
    func singleStepMatchesCPU() {
        autoreleasepool {
            ...
            let cpuY = cpuRefStep(
                x: x, w: w, b: b, state: &cpuState,
                nChannels: nChannels, kernelSize: kernelSize)
            let (gpuState, gpuY) = gpuStep(
                x: x, w: w, b: b, initialState: initial,
                nChannels: nChannels, kernelSize: kernelSize)
    ...
    func bf16Inputs() {
        autoreleasepool {
            ...
            let yT = Tensor.empty(shape: [nChannels], dtype: .bf16)
            yT.zero()
            runAndWait { cb in
                Ops.conv1dCausalStep(
                    x: xT, w: wT, b: bT,
