...
struct GDNStateCacheTests {
    ...
    func initShape() {
        autoreleasepool {
            let cache = GDNStateCache(numValueHeads: 8, valueHeadDim: 64,
                                      keyHeadDim: 64)
    ...
    func swapPingPongs() {
        autoreleasepool {
            let cache = GDNStateCache(numValueHeads: 1, valueHeadDim: 2,
                                      keyHeadDim: 32)
    ...
    func resetClears() {
        autoreleasepool {
            let cache = GDNStateCache(numValueHeads: 1, valueHeadDim: 1,
                                      keyHeadDim: 32)
    ...
    func totalBytes() {
        autoreleasepool {
            let c1 = GDNStateCache(numValueHeads: 8, valueHeadDim: 64,
                                   keyHeadDim: 64)
            let c2 = GDNStateCache(numValueHeads: 8, valueHeadDim: 64,
                                   keyHeadDim: 64)
    ...
    func singleStepMatchesCPU() {
        autoreleasepool {
            ...
            let cpuY = cpuRef(q: q, k: k, v: v, g: g, beta: beta,
                              state: &cpuState, tSteps: tSteps)
            let (gpuState, gpuY) = gpuRun(q: q, k: k, v: v, g: g, beta: beta,
