...
//
// Slow integration test for Qwen 2-VL (the dynamic-resolution full-attention
// ViT + Qwen 2 text backbone `Qwen2VLForConditionalGeneration` checkpoint).
//
// Verifies the vision path end-to-end on a real checkpoint: the Qwen 2-VL
// vision tower loads, runs its full-attention + M-RoPE forward and
// patch-merger, the cross-modal splice injects the merged image tokens, and
// the fused stream decodes coherent text through the Qwen 2 backbone.
//
// Uses the mlx-community 2B-Instruct-4bit conversion (smallest published
// Qwen 2-VL). The checkpoint MUST load — a load failure fails the test.
// Now also covers the video path for the same checkpoint (merged from the
// prior Qwen2VisionVideoIntegrationTests.swift).
...
@Suite("Qwen2 Vision Integration (image + video)", .serialized)
struct Qwen2VisionIntegrationTests {
    ...
    @Test("load — Qwen 2-VL checkpoint loads with vision capability")
    ...
    func imageTextGeneration() async throws {
        ...
        expectCoherentOutput(generated, minTokens: 8,
                             label: "Qwen 2-VL image+text")
        let text = m.tokenizer.decode(tokens: generated, skipSpecialTokens: true)
        print("Qwen 2-VL generated: \(text)")
        VisionTestHelpers.expectMentionsDog(text, label: "Qwen 2-VL")
    }
