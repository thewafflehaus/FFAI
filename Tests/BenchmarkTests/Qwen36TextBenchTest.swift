...
//
// Qwen3.6-35B-A3B benchmark suite — micro-bench coverage for the
// Qwen3.6 hybrid (GDN + attention + MoE) engine. Separate from
// `Qwen36TextIntegrationTests` (which covers behavioural correctness
// only — load, interfaces, errors, forward + coherent output) because
...
//
// Coverage:
//   - benchShort / bench4k / bench32k — prefill + decode steady-state
//     throughput at progressively longer contexts.
//   - forwardManyBench{,128,512,2K} — batched forwardMany vs per-token
//     forward speedup at T ∈ {32, 128, 512, 2048}.
//   - forwardManyEquivalence — top-1 logit equivalence between the
//     batched forwardMany path and the per-token forward path
//     (correctness gate for the batched path).
//   - firstTokenForward — single greedy first-token decode with
//     printed top-5 for manual mlx-swift-lm baseline comparison.
...
struct Qwen36TextBenchTest {
    ...
    /// Test target — the cached `mlx-community/Qwen3.6-35B-A3B-4bit`
    /// MoE checkpoint. Switching to the HF identifier (vs the earlier
    /// hard-coded `/Users/.../Qwen3.6-35B-A3B-4bit` local path) means
    /// the suite runs against the same checkpoint on any developer's
    /// machine once they've fetched it via `ffai download`.
    private static let modelId = "mlx-community/Qwen3.6-35B-A3B-4bit"
    ...
    func benchShort() async throws {
        ...
        // 4K/32K prefill takes prohibitively long. A 32-token prompt +
        // 32 decode steps is enough to see steady-state decode tps
    ...
    /// repeating a base sentence, then measure prefill + decode
    /// throughput.
    private func runBench(targetPromptTokens: Int, decodeSteps: Int, label: String) async throws {
        ...
        let m = try await ModelLoadLock.shared.loadSerially {
            try await Model.load(Self.modelId, options: opts)
        }
