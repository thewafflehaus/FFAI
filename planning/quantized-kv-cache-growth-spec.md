...
`AffineQuantizedKVCache` (`Sources/FFAI/KVCache/KVCache.swift`) holds **6** per-layer buffers, each `[nKVHeads, capacity, width]` with a different inner width:
...
- Per-layer **packed codebook indices** `[nKVHeads, capacity, packedWidth]` u32 (K and V can have *different* bit widths → different packedWidths) + **norms** `[nKVHeads, capacity]` f32 — both restride on growth.
...
2. **`AffineQuantizedKVCache`**: mirror the raw cache's growth structure —
   - 6 buffers → `private(set) var`; the stored `capacity` already satisfies `LayerCacheProtocol` (the old `maxSeq` alias is gone — see the cache→`capacity` / init→`contextLength` rename); add `contextCeiling`; store `device` + a retired-buffer list.
