...
**Resolution (FFAI test).** Adjust `Quantized3bitIntegrationTests` to match precedent — either relax the diversity floor (Qwen3.5-0.8B used 0.05) or drop the coherence assertion in favour of a finite-logits + dispatch smoke (2-bit-test style), with a comment pointing here. The int3 kernels need no change; the new production-shape correctness cells are the durable signal that the path is correct.
